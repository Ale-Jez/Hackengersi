...
<body>
        ...
        <div>
            <section>
                <div>
                    ...
                    <div>
                      <h2 class="tittle-h2">OTHER FUNCTIONS</h2>
                      <div class="two-btn">  
                        <div>
                 ...
                 <div>
                    ...
                    <div style="margin-bottom: 20px;">
                        <label>AP_SSID:</label>
                        <input id="ap_ssid" value="RoArm-M3" style="width:158px; margin: 0 12px; padding:6px; border-radius:6px; border:1px solid #555; background:#f1f2f6; color:#2f3542;" />
                        <label>AP_PWD:</label>
                        <input id="ap_pwd" value="12345678" style="width:158px; margin: 0 12px; padding:6px; border-radius:6px; border:1px solid #555; background:#f1f2f6; color:#2f3542;" />
                    </div>
                    <p>STA STATUS:</p>
                    <p id="sta_status">Disconnected</p>
                    <div style="margin-bottom: 20px;">
                        <label>STA_SSID:</label>
                        <input id="sta_ssid" style="width:152px; margin: 0 12px; padding:6px; border-radius:6px; border:1px solid #555; background:#f1f2f6; color:#2f3542;" />
                        <label>STA_PWD:</label>
                        <input id="sta_pwd" type="password" style="width:152px; margin: 0 12px; padding:6px; border-radius:6px; border:1px solid #555; background:#f1f2f6; color:#2f3542;" />
                    </div>
                    <div>
            ...
            <section>
                <div>
                    <div class="fb-input-info">
                        <h2 class="tittle-h2">Feedback information</h2>
                        <div class="sec-infotext">
                            <p id="GetInfoText">Json feedback information shows here.</p>
                        </div>
                        <div class="feedb-p">
                            <div>
                        ...
                        <div class="Servo-set">
                            ...
                            <div class="info-box json-cmd-info">
                                <div>
                                    <p>CMD_RESET_EMERGENCY</p>
                                    <p class="cmd-value">{"T":999}</p>
                                </div>
                                <button class="w-btn">INPUT</button>
                            </div>
                            <div class="info-box json-cmd-info">
    ...
    let isOvertemperature = false;
    let isOvervoltage=false;
...
var sock = new WebSocket("ws://" + window.location.host + "/ws");
           ...
           var jsonResponse = JSON.parse(ev.data);
                   ...
                   console.log("jsonRecv", jsonResponse);
                    if (jsonResponse.T == 50) 
                      {
                        document.getElementById("sta_status").innerHTML = jsonResponse.ssid  + jsonResponse.sta;
                     }
                     else if (jsonResponse.T == -15)
                     {
                      document.getElementById("anglectrl-t1").innerHTML = Number(jsonResponse.b || 0).toFixed(2);
                      document.getElementById("anglectrl-t2").innerHTML = Number(jsonResponse.s || 0).toFixed(2);
                      document.getElementById("anglectrl-t3").innerHTML = Number(jsonResponse.e || 0).toFixed(2);
                      document.getElementById("anglectrl-t4").innerHTML = Number(jsonResponse.t || 0).toFixed(2);
                      document.getElementById("anglectrl-t5").innerHTML = Number(jsonResponse.r || 0).toFixed(2);
                      document.getElementById("anglectrl-t6").innerHTML = Number(jsonResponse.g || 0).toFixed(2);

                      document.getElementById("coordCtrl-t1").innerHTML = Number(jsonResponse.x || 0).toFixed(2);
                      document.getElementById("coordCtrl-t2").innerHTML = Number(jsonResponse.y || 0).toFixed(2);
                      document.getElementById("coordCtrl-t3").innerHTML = Number(jsonResponse.z || 0).toFixed(2);
                      document.getElementById("coordCtrl-t4").innerHTML = Number(jsonResponse.tit || 0).toFixed(2);
                      document.getElementById("coordCtrl-t5").innerHTML = Number(jsonResponse.r || 0).toFixed(2);
                      document.getElementById("coordCtrl-t6").innerHTML = Number(jsonResponse.g || 0).toFixed(2);
                   
                        if (jsonResponse.Stalltor) {
                        showWarn("tor", "⚠️ 检测到舵机电流异常，已禁用 UI 按键，请立即断电，并检查机械结构是否存在卡滞、阻挡或负载过大的情况。<br>Servo overcurrent detected. UI controls have been disabled. Please power off the device immediately and check whether the mechanical structure is jammed, blocked, or overloaded.");
                        isOvercurrent=true;
                        } else
                        {
                          hideWarn("tor");
                         ...
                         if (jsonResponse.Stalltep) {
                        showWarn("temp", "⚠️ 检测到舵机温度过高，已禁用 UI 按键，请断电并等待设备冷却后再继续使用。<br>Servo overtemperature detected. UI controls have been disabled. Please power off the device and wait for it to cool down before continuing use.");
                        isOvertemperature=true;
                        } else {
                            hideWarn("temp");
                            isOvertemperature=false;
                        }
                        if (jsonResponse.Stallvol==1) {
                            showWarn("voltage", "⚠️ 检测到舵机输入电压异常偏高，已禁用 UI 按键，请检查电源输出电压是否在设备允许范围内。<br>Servo input overvoltage detected. UI controls have been disabled. Please check whether the power supply output voltage is within the allowable operating range of the device.");
                            isOvervoltage=true;
                        } else if (jsonResponse.Stallvol==2)
                        {
                            showWarn("voltage", "⚠️ 检测到设备工作电压过低，舵机无法正常工作。请使用设备的工作电压进行供电。<br>Low operating voltage detected. The servo cannot operate properly. Please use the device's specified operating voltage for power supply.","#FFC107");
                            isOvervoltage=false;
                        }
                        else
                        {
                     ...
                     {
                      document.getElementById("GetInfoText").innerHTML =ev.data;
                     }
        ...
        function jsonSend() {
          if (!isOvertemperature&&!isOvercurrent&&!isOvervoltage)
          {
       ...
       function showWarn(id, text, color="red") {
          ...
          if (!div) {
              ...
              document.getElementById("warnContainer").appendChild(div);
          }
        ...
        function opera1(x, y) {
            ...
            if (y) {
                ...
                document.getElementById(x).value = rs - 1;05
            }
       }
...
<script>
    ...
    slider.addEventListener("input", function() {
        ...
        if (sendSucceed&&!isOvertemperature&&!isOvercurrent&&!isOvervoltage) {
            moveto(in_x, in_y);
            sendSucceed = false;
        }
    });
