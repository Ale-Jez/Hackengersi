...
typedef struct struct_message {
  uint32_t magic;// 固定值 0xA5A5A5A5，保证是合法数据
  byte devCode;
  float base;
  float shoulder;
  float elbow;
...
//espNowMessage.magic=0xA5A5A5A5;
...
// void OnDataRecv(const unsigned char* mac, const unsigned char* incomingData, int len) {
  ...
  if (espNowMegsRecv.magic != 0xA5A5A5A5) {
    Serial.println("Invalid data, ignore");
    return;
  }
