...
const char* targetStaSsid = "none";
const char* targetStaPwd = "none";
...
bool wifiModeSTA(const char* input_ssid, const char* input_password) {
  ...
  targetStaSsid = input_ssid;
  targetStaPwd = input_password;
  wifiConnecting = 1;
   return true;
/*
...
bool wifiModeAPSTA(const char* input_ap_ssid, const char* input_ap_password, const char* input_sta_ssid, const char* input_sta_password) {
  ...
  targetStaSsid = input_sta_ssid;
  targetStaPwd = input_sta_password;
  ...
  if (InfoPrint == 1) {Serial.print("STA/AP+STA mode starts: connecting to ");
	...
	jsonInfoHttp["ap_ssid"] = ap_ssid;
	jsonInfoHttp["ap_password"] = ap_password;
	jsonInfoHttp["sta_ip"] = localIP.toString();
	return true; 
  */
}
...
void handleWifiConnection()
{
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        if (wifiConnecting==2)
        WIFI_CURRENT_MODE = 3;
        else if(wifiConnecting==1)
        WIFI_CURRENT_MODE = 2;
        getIPAddress(WIFI_CURRENT_MODE);
        sta_ssid = targetStaSsid;
        sta_password = targetStaPwd;
        wifiConnecting = 0;
