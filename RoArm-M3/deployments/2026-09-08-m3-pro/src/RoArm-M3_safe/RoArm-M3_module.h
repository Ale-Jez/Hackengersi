...
bool getFeedback(byte servoID, bool returnType) {
  if(st.FeedBack(servoID)!=-1) {
    ...
    servoFeedback[servoID - 11].current = st.ReadCurrent(-1)*6.5;
    servoFeedback[servoID - 11].temper = st.ReadTemper(-1);
    servoFeedback[servoID - 11].mode = st.ReadMode(servoID);
    servoFeedback[servoID - 11].Torquestatus=st.readByte(servoID,SMS_STS_TORQUE_ENABLE);
    if(!returnType){
    ...
    servoFeedback[servoID - 11].current=0;
    servoFeedback[servoID - 11].temper=0;
    servoFeedback[servoID - 11].Torquestatus=0;
...
void servoTorqueCtrl1(byte servoID, u8 enableCMD){
...
void Move_to_location()
{
  ...
  servoTorqueCtrl1(SHOULDER_DRIVEN_SERVO_ID, 0);

  // move SHOULDER_DRIVING_SERVO to middle position.
  st.WritePosEx(SHOULDER_DRIVING_SERVO_ID, ARM_SERVO_MIDDLE_POS-1024, ARM_SERVO_INIT_SPEED, ARM_SERVO_INIT_ACC);

  // check SHOULDER_DRIVEING_SERVO position.
  waitMove2Goal(SHOULDER_DRIVING_SERVO_ID, ARM_SERVO_MIDDLE_POS-1024, 80);

  // wait for the jitter to go away.
  delay(1200);
  servoTorqueCtrl1(SHOULDER_DRIVEN_SERVO_ID, 1);
  delay(10);
  st.WritePosEx(WRIST_SERVO_ID, ARM_SERVO_MIDDLE_POS, ARM_SERVO_INIT_SPEED, ARM_SERVO_INIT_ACC);
  waitMove2Goal(WRIST_SERVO_ID, ARM_SERVO_MIDDLE_POS, 80);

  st.WritePosEx(ROLL_SERVO_ID, ARM_SERVO_MIDDLE_POS, ARM_SERVO_INIT_SPEED, ARM_SERVO_INIT_ACC);
  waitMove2Goal(ROLL_SERVO_ID, ARM_SERVO_MIDDLE_POS, 80);

  st.WritePosEx(ELBOW_SERVO_ID, ARM_SERVO_MIDDLE_POS+1024, ARM_SERVO_INIT_SPEED, ARM_SERVO_INIT_ACC);
  waitMove2Goal(ELBOW_SERVO_ID, ARM_SERVO_MIDDLE_POS+1024, 80);
...
void waitMove2Goal(byte InputID, s16 goalPosition, s16 offSet){
  while(servoFeedback[InputID - 11].pos < goalPosition - offSet || 
        servoFeedback[InputID - 11].pos > goalPosition + offSet){
          ...
          // Serial.print("pos");
          // Serial.println((servoFeedback[InputID - 11].pos));
          // Serial.print("status");
          // Serial.println((servoFeedback[InputID - 11].status));
          // Serial.print("FeedBack");
          // Serial.println((st.FeedBack(InputID)));

    if (!servoFeedback[InputID - 11].status) {
      servoTorqueCtrl1(254, 0);
      break;
    }
...
int RoArmM3_elbowJointCtrlRad(byte returnType, double radInput, u16 speedInput, u8 accInput) {
  ...
  goalPos[3] = constrain(computePos, 1024, 3071);
...
void RoArmM3_infoFeedback() {
  ...
  jsonInfoHttp["torswitchB"] = servoFeedback[BASE_SERVO_ID - 11].Torquestatus;
  jsonInfoHttp["torswitchS"] = servoFeedback[SHOULDER_DRIVING_SERVO_ID - 11].Torquestatus;
  jsonInfoHttp["torswitchE"] = servoFeedback[ELBOW_SERVO_ID - 11].Torquestatus;
  jsonInfoHttp["torswitchT"] = servoFeedback[WRIST_SERVO_ID - 11].Torquestatus;
  jsonInfoHttp["torswitchR"] = servoFeedback[ROLL_SERVO_ID - 11].Torquestatus;
  jsonInfoHttp["torswitchG"] = servoFeedback[GRIPPER_SERVO_ID - 11].Torquestatus;
  inaDataUpdate();
...
void Stall_detection()
{
  ...
  //   get_torque(i);
  // }
  if(servoFeedback[0].current<Protective_current&&servoFeedback[1].current<Protective_current&&servoFeedback[2].current<Protective_current&&servoFeedback[3].current<Protective_current&&servoFeedback[4].current<Protective_current&&servoFeedback[5].current<Protective_current&&servoFeedback[6].current<Protective_current)
  {
    Stall_flag=false;
  }else
  {
    Stall_flag=true;
    Serial.println("Servo overcurrent detected. Please power off the device immediately and check for mechanical blockage, obstruction, or excessive load.");
  }
  // if(servoFeedback[0].temper<Protective_temp&&servoFeedback[1].temper<Protective_temp&&servoFeedback[2].temper<Protective_temp&&servoFeedback[3].temper<Protective_temp&&servoFeedback[4].temper<Protective_temp)
  // {
  //   Temp_flag=false;
  // }else
  // {
  //   Temp_flag=true;
  //   Serial.println("Servo temperature is too high. Please power off the device and allow it to cool down before further use.");
  // }
  if((((int)(loadVoltage_V * 100))<Protective_voltage)&&(((int)(loadVoltage_V * 100))>Protective_voltage_min))
  {
    Voltage_flag=0;
  }else if(((int)(loadVoltage_V * 100))>Protective_voltage)
  {
    Voltage_flag=1;
    Serial.println("Servo input voltage is too high. Please verify that the power supply voltage is within the specified operating range.");
  }else if(((int)(loadVoltage_V * 100))<Protective_voltage_min)
  {
    Voltage_flag=2;
    Serial.println("Low operating voltage detected. The servo cannot operate properly. UI controls have been disabled. Please use the device's operating voltage for power supply.");
  }
  // Voltage_flag=true;
  // Stall_flag=true;
  // Temp_flag=true;
  ...
  // //RoArmM2_allJointAbsCtrl(radB,radS,radE,radG,0,10);
  // servoTorqueCtrl1(254,0);
  // servoTorqueCtrl1(254,1);
  //  // RoArmM2_baseCoordinateCtrl(lastX, lastY, lastZ, lastT);
  //   // RoArmM2_goalPosMove();
  // }
}
