...
"""Ramie SO-101 - WSZYSTKO W JEDNYM PLIKU.
  ...
  python ramie.py                 (albo przycisk Run w VS Code) okno kamery + SLEDZENIE butelki kaucyjnej:
                             ramie trzyma jej kod na srodku obrazu, puszcza gdy sie oddali
                             albo zgubi, przelacza na inna kaucyjna, jesli jest blizej (T wl/wyl)
                             + TRACKING HISTORY: zapamietuje pozycje butelek w czasie (Y zapis)
  python ramie.py --bez-kamery    sterowanie z terminala + ogladanie puszki pod ENTER, bez kamery
  python ramie.py --mock          bez ramienia (sama kamera, ramie stoi)
  (port adaptera znajduje sie sam; mozna podac np. COM5)
...
# =============================================================================
# TRACKING HISTORY CLASSES - for persistent tracking across time
# =============================================================================
...
POMOC = (
    "WASD/IJKL/UO ruch | SPACJA chwyc/pusc | Z/X chwytak | 1 2 3 predkosc | F stop\n"
    "ENTER ogladaj puszke | T sledzenie (kamera) | P zapisz poze skanu | R usun pozy skanu | M zapisz dom | H do domu | Y zapis tracking | Q koniec"
)
...
class Sledzenie:
    """Trzyma kod kaucyjny na srodku obrazu z historia sledzenia w czasie.
    Puszcza, gdy za daleko albo zgubiony; przelacza sie na inna butelke kaucyjna,
    jesli jest wyraznie blizej. Uzywa TrackingHistory do sledzenia w czasie."""

    def __init__(self, tracking_history=None):
        self.cel = None
        self.rozmiar = 0.0
        self.widziany = 0.0
        self.xy = None
        self.tracking_history = tracking_history or TrackingHistory()
        self._reset()

    def krok(self, kody, kaucje, szer, wys, st, dt):
        """Aktualizuje cel i przesuwa st.target. Zwraca komunikat albo None."""
        teraz = time.time()
        self.tracking_history.cleanup(teraz)
        
        kaucyjne = []
        for k in kody:
            if k["ean"]:
                ean = k["ean"]
                is_kaucja = kaucje.sprawdz(ean)
                if is_kaucja and k["rozmiar"] >= MIN_SZEROKOSC:
                    kaucyjne.append(k)
                    name = kaucje.nazwy.get(ean, "")
                    self.tracking_history.update(
                        ean, name, teraz, k["cx"], k["cy"], k["rozmiar"], st.here["pan"], st.here["wflex"]
                    )
            elif k["ean"] is None and self.cel and self.xy:
                distance = abs(k["cx"] - self.xy[0]) + abs(k["cy"] - self.xy[1])
                if distance < SLEDZ_PROMIEN * szer:
                    if self.cel in self.tracking_history.items:
                        self.tracking_history.update(
                            self.cel, "", teraz, k["cx"], k["cy"], k["rozmiar"], st.here["pan"], st.here["wflex"]
                        )

        kaucyjne.sort(key=lambda k: k["rozmiar"], reverse=True)
        najblizszy = kaucyjne[0] if kaucyjne else None
        msg = None
        
        biezacy = None
        if self.cel:
            biezacy = next((k for k in kody if k["ean"] == self.cel), None)
            if biezacy is None and self.xy:
                bliskie = [k for k in kody if k["ean"] is None and
                           abs(k["cx"] - self.xy[0]) + abs(k["cy"] - self.xy[1]) < SLEDZ_PROMIEN * szer]
                biezacy = min(bliskie, key=lambda k: abs(k["cx"] - self.xy[0]) + abs(k["cy"] - self.xy[1]),
                              default=None)
            if biezacy is None and self.cel in self.tracking_history.items:
                predicted = self.tracking_history.predict_position(self.cel, teraz)
                if predicted:
                    predicted_x, predicted_y = predicted
                    bliskie_pred = [k for k in kody if k["ean"] is None and
                                   abs(k["cx"] - predicted_x) + abs(k["cy"] - predicted_y) < SLEDZ_PROMIEN * szer * 2]
                    biezacy = min(bliskie_pred, key=lambda k: abs(k["cx"] - predicted_x) + abs(k["cy"] - predicted_y),
                                  default=None)
        ...
        if self.cel is None:
            if najblizszy:
                ...
                msg = f"SLEDZE {self.cel}"
                if self.cel in self.tracking_history.items:
                    self.tracking_history.items[self.cel].status = "active"
        elif biezacy is not None:
            if biezacy["ean"] and biezacy["rozmiar"] < MIN_SZEROKOSC:
                msg, self.cel, biezacy = f"{self.cel} ODDALILA SIE - puszczam", None, None
                if self.cel and self.cel in self.tracking_history.items:
                    self.tracking_history.mark_lost(self.cel)
            elif biezacy["ean"] and najblizszy and najblizszy["ean"] != self.cel and \
                    najblizszy["rozmiar"] > biezacy["rozmiar"] * PRZELACZ_GDY:
                msg = f"INNA BLIZEJ: {self.cel} -> {najblizszy['ean']}"
                self.cel, biezacy = najblizszy["ean"], najblizszy
            elif biezacy["ean"] is None:
                pass
        elif najblizszy:
            msg = f"INNA BUTELKA: {self.cel} -> {najblizszy['ean']}"
            self.cel, biezacy = najblizszy["ean"], najblizszy
        ...
        if self.cel and biezacy:
            if biezacy["ean"]:
                name = kaucje.nazwy.get(biezacy["ean"], "")
                self.tracking_history.update(
                    self.cel, name, teraz, biezacy["cx"], biezacy["cy"],
                    biezacy["rozmiar"], st.here["pan"], st.here["wflex"]
                )
            elif self.cel in self.tracking_history.items:
                self.tracking_history.update(
                    self.cel, "", teraz, biezacy["cx"], biezacy["cy"],
                    biezacy["rozmiar"], st.here["pan"], st.here["wflex"]
                )
        ...
        if self.cel and self.cel in self.tracking_history.items:
            item = self.tracking_history.items[self.cel]
            vx, vy = item.get_velocity()
            self._current_velocity = (vx, vy)
    ...
    def _szukaj(self, st, teraz):
        """Kod chwilowo niewidoczny: jedz dalej tam, dokad uciekal z uzyciem predykcji."""
        if not self._hist or teraz < self._nastepny or not hasattr(self, "g"):
            return
        od_utraty = teraz - self.widziany
        if od_utraty > SLEDZ_SZUKAJ_CZAS or self._szukano >= SLEDZ_SZUKAJ_MAX:
        ...
        if hasattr(self, "_current_velocity") and self._current_velocity:
            vx, vy = self._current_velocity
            horyzont = min(od_utraty + SLEDZ_PAUZA, 1.0)
            if self._hist:
                last_x = self._hist[-1][1]
                last_y = self._hist[-1][2]
                przew_x = last_x + vx * horyzont
                przew_y = last_y + vy * horyzont
            else:
                przew_x, przew_y = 0.0, 0.0
        else:
            (t0, x0, y0), (t1, x1, y1) = self._hist[0], self._hist[-1]
            vx, vy = ((x1 - x0) / (t1 - t0), (y1 - y0) / (t1 - t0)) if t1 - t0 > 0.05 else (0.0, 0.0)
            horyzont = min(od_utraty + SLEDZ_PAUZA, 1.0)
            przew = {"x": max(-1.5, min(1.5, x1 + vx * horyzont)), "y": max(-1.5, min(1.5, y1 + vy * horyzont))}
            przew_x, przew_y = przew["x"], przew["y"]
        ...
        for o, staw in stawy.items():
            if o == "x" and abs(przew_x) > SLEDZ_CEL:
                d = -SLEDZ_KROK * przew_x / self.g["x"]
                d = max(-min(SLEDZ_MAX_KROK, zostalo), min(min(SLEDZ_MAX_KROK, zostalo), d))
                if abs(d) >= SLEDZ_MIN_KROK:
                    st.target[staw] += d
                    ruch[staw] = round(d, 1)
            elif o == "y" and abs(przew_y) > SLEDZ_CEL:
                d = -SLEDZ_KROK * przew_y / self.g["y"]
                d = max(-min(SLEDZ_MAX_KROK, zostalo), min(min(SLEDZ_MAX_KROK, zostalo), d))
                if abs(d) >= SLEDZ_MIN_KROK:
                    st.target[staw] += d
                    ruch[staw] = round(d, 1)
        ...
        if ruch:
            ...
            self._ostatni_ruch = None
            self._probki = []
            _log(f"SZUKAM cel={self.cel} przewidziany x={przew_x:+.2f} y={przew_y:+.2f} ruch={ruch} "
                 f"lacznie {self._szukano:.0f} st. velocity=({vx:.1f}, {vy:.1f})")
    ...
    def _steruj(self, st, msg, teraz):
        """Popraw i poczekaj: mediana kilku klatek -> jeden ruch -> pauza."""
        if not hasattr(self, "_probki"):
            self._reset()
        ...
        if self.cel and self.cel in self.tracking_history.items:
            item = self.tracking_history.items[self.cel]
            tracking_info = f" tracking={len(item.positions)}pts"
            if item.name:
                tracking_info += f" '{item.name}'"

        _log(f"cel={self.cel} x={bx:+.2f} y={by:+.2f} {'odczyt' if self.blad[2] else 'prostokat'} "
             f"pan={st.here['pan']:.1f} wflex={st.here['wflex']:.1f} "
             f"ruch={ {stawy[o]: round(d, 1) for o, d in ruch.items()} } "
             f"czulosc x={self.g['x']:+.3f} y={self.g['y']:+.3f}{' W CELU' if self._w_celu else ''}{tracking_info}")
        return msg

    def get_tracking_summary(self):
        """Get a summary of all tracked items."""
        summary = {}
        for ean, item in self.tracking_history.items.items():
            summary[ean] = {
                "name": item.name,
                "status": item.status,
                "first_seen": item.first_seen,
                "last_seen": item.last_seen,
                "total_positions": len(item.positions),
                "pickup_count": item.pickup_count,
                "tracking_time": item.total_tracking_time
            }
        return summary

    def get_current_target_info(self):
        """Get detailed info about current target."""
        if not self.cel:
            return None
        item = self.tracking_history.items.get(self.cel)
        if not item:
            return None
        return {
            "ean": item.ean,
...
def tryb_kamera(port=None, mock=False):
    ...
    # msvcrt is Windows-only, skip keyboard input on Linux
    ...
    tracking_history.load()
    sled = Sledzenie(tracking_history)
    sledz = SLEDZ
    t_klatki = time.time()
    srv = serwer_http(arm)
    print(f"Pad: {'PODLACZONY' if st.pad_ok else 'brak (tylko klawiatura)'}")
        ...
        while True:
            ...
            for c in wejscie:
                ...
                if c == "\r":
                    ...
                    if tracking_history.save():
                        pokaz("Zapisano historię śledzenia")
                    else:
                        pokaz("Błąd zapisu historii")
                else:
                    st.klawisz(c)
            ...
            summary = sled.get_tracking_summary()
            active_count = sum(1 for s in summary.values() if s["status"] == "active")
            picked_count = sum(1 for s in summary.values() if s["status"] == "picked_up")
            gora += f" | Tracked: {active_count} active, {picked_count} picked"
            h, w_ = wys, szer
            cv2.drawMarker(klatka, (szer // 2, wys // 2), (255, 255, 255), cv2.MARKER_CROSS, 24, 1)
            cv2.rectangle(klatka, (0, 0), (w_, 28), (0, 0, 0), -1)
            cv2.putText(klatka, gora, (6, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1)
            cv2.rectangle(klatka, (0, h - 46), (w_, h), (0, 0, 0), -1)
            cv2.putText(klatka, st.opis()[:90], (6, h - 28), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200, 255, 200), 1)
            cv2.putText(klatka, "T sledzenie | ENTER ogladaj | WASD IJKL UO ruch | SPACJA chwyt | P/M/H/Y | Q",
                        (6, h - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (200, 200, 200), 1)
            if komunikat and time.time() < komunikat_do:
                cv2.putText(klatka, komunikat, (6, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            
            # Display tracking info for current target
            target_info = sled.get_current_target_info()
            if target_info:
                info_text = f"Target: {target_info['ean']}"
                if target_info['name']:
                    info_text += f" ({target_info['name']})"
                info_text += f" | V:({target_info['velocity'][0]:.0f}, {target_info['velocity'][1]:.0f})px/s"
                cv2.putText(klatka, info_text, (6, h - 70),
